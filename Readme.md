...
* Fused API 는 구글 플레이 서비스에 종속적입니다. 모든 안드로이드 폰에 플레이 서비스가 포함되어 있는 것은 아니니 주의해야 합니다. 물론 Play Store 앱이 업데이트되면 Fused Location 서비스도 같이 최신 상태로 업데이트 된다는 이점도 있습니다.

__LocationManager 문제__

* 2개 이상의 프로바이더로부터 Best Location을 취해야 하는 경우, 로직을 직접 구현해야 하는 부담이 있습니다. Fused API는 이 로직을 직접 구현하지 않아도 되는 장점이 있습니다. 
